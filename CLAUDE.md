...
- All three controllers (`Pizza`, `Customer`, `Order`) now consistently return response DTOs (`PizzaResponseDto`, `CustomerResponseDto`, `OrderResponseDto`) from every endpoint — no controller exposes a raw JPA entity anymore.
- All three controllers' `add()` endpoints now take a dedicated `*CreateDto` (`PizzaCreateDto`, `CustomerCreateDto`, `OrderCreateDto`, plus `OrderItemCreateDto` for nested items) as `@RequestBody` with `@Valid`, instead of the raw entity — callers can no longer set fields that shouldn't be externally controlled (e.g. `idOrder`, `createdDate`).
- Input validation now exists via Bean Validation (`@NotBlank`, `@NotNull`, `@Positive`, etc.) on the `*CreateDto` records, enforced with `@Valid` on the `add()` endpoints.
- `web/exception/GlobalExceptionHandler.java` (`@RestControllerAdvice`) catches `MethodArgumentNotValidException` and returns a consistent `{field: message}` error body for validation failures. It does **not** yet cover the "not found" case — controllers still return `null`/404 manually via `if (x != null) ... else notFound()` rather than throwing/catching a dedicated not-found exception, so that part of the error-response shape is still inconsistent.
- Test coverage is still limited to the two tests noted above — `OrderService`, `PizzaService`, `CustomerService`, and all three controllers remain untested.
